...
```{r}
# ##summed per site
hab.sum <- read_rds("E:/Archive/2_pipeline/2_HabitatSum/out/hab_sum_site.RDS")
counts <- read_rds("E:/Archive/2_pipeline/3_Detectability/out/sitelevel_counts.RDS")
...
hab.sum<-hab.sum%>%filter(siteID%in%counts$siteID)%>%filter(buffer=="500")
...
all.equal(unique(hab.sum$siteID), rownames(counts.m))
...
table(filter(hab.sum, buffer == 150) %>% pull(h_burn) > 0.1) ## 18/145, 12% of sites > 10% burned
...
pairs(hab.sum %>% filter(buffer == 1000) %>% select(d_surface, d_lden_all, h_forest, h_open, h_wet,  wden, mn_june_tmin,  elev, lat, lon),
      upper.panel=panel.smooth,
      diag.panel = panel.hist,
...
tmp[tmp<.20*n.sites] ## will loose all the SAR species. 
tmp[tmp> .75*n.sites] ##require birds to be detected at <80% of sites? For abundance, highend shouldn't matter 
counts.spec <- counts.spec[tmp>.2*n.sites] ##require species to be detected at > 20% of sites (29) 

counts.spec <- lapply(counts.spec, function(x) inner_join(x, hab.sum %>% filter(wetland.layer & siteID != "371499-B")))

source("E:/Archive/1_code/multifit.R") ##https://doi.org/10.1007/s10980-018-0657-5;  https://github.com/phuais/multifit/blob/master/multifit.R,
...
saveRDS(counts.l, file.path(pipeline, "store", "counts_scaled.rds"))
saveRDS(var.sel, file.path(pipeline, "store", "selected_scales.rds"))
...
```{r}
counts.l <- readRDS(file.path("E:/Archive",pipeline, "store", "counts_scaled.rds"))
var.sel <- readRDS(file.path("E:/Archive",pipeline, "store", "selected_scales.rds"))
...
```{r}
saveRDS(counts.spec, file.path("E:/Archive",pipeline, "store", "counts_simp.RDS"))
saveRDS(hab.sum, file.path("E:/Archive",pipeline, "store", "hab.sum_simp.RDS"))
