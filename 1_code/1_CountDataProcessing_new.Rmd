...
project_ids <- c(577,2078,2782,446) ## ADD 446 back once i get access!!!
...
write_rds(aru, file.path('E:/Archive/',pipeline, "tmp", "wt_2024_11_12.RDS")) ## store a backup copy of the download file so download can be skipped in future
aru <- read_rds(file.path('E:/Archive/',pipeline, "tmp", "wt_2024_11_12.RDS"))
...
aru_noise <- rbind(read_csv("E:/Archive/0_data/SB2023_Tasks_20240219.csv"), read_csv("E:/Archive/0_data/SB2021_Tasks_20240219.csv"), read_csv("E:/Archive/0_data/South Beringia Priority Place Cumulative Effects 2024_Tasks_202423.csv"), read_csv("E:/Archive/0_data/CWS1718_Tasks_20231207.csv"))
...
aru_noise <- aru_noise %>% filter(status=="Transcribed") %>%
  ...
  select(location, task_id = internal_task_id, recording_date_time = recordingDate, observer = transcriber, task_method = method, obsNoise) %>%mutate(recording_date_time=ymd_hms(recording_date_time))%>% ungroup()
...
aru[[1]] <- aru[[1]] %>% mutate(recording_date_time=ymd_hms(recording_date_time),site = str_split_i(location, "-", 1), 
                      station = str_split_i(location, "-", 2))
aru[[2]] <- aru[[2]] %>% mutate(recording_date_time=ymd_hms(recording_date_time),site = str_split_i(location, "-", 1), 
                      station = str_split_i(location, "-", 2))
aru[[3]] <- aru[[3]] %>% mutate(recording_date_time=ymd_hms(recording_date_time),site = str_split_i(location, "-", 1), 
                      station = str_split_i(location, "-", 2))
aru[[4]] <- aru[[4]] %>% mutate(recording_date_time=ymd_hms(recording_date_time),site = paste(str_split_i(location, "-", 3), 
...
```{r PC}
## Site metadata
sites2021 <- read_csv("E:/Archive/0_data/EW2021-Sites-correctedmjb.csv")
...
sites2021$time <- dmy_hms(paste(sites2021$date, hour(sites2021$time),
                          ...
                          tz = "Canada/Yukon")
sites2021$pickup_time <- dmy_hms(paste(sites2021$pickup_date, sites2021$pickup_time),
                                   tz = "Canada/Yukon")


###ECCC data PC data ----
eccc <- read_csv("E:/Archive/0_data/CWS_WH_LANDBIRD_PCS_2017-2019.csv")
###some coordinates were incorrect in the database when provided. 
##correct coordinates
coord.cor <- read_csv("E:/Archive/0_data/CWS_CoordCor.csv") ##provided by logan (feb 2023)
...
eccc <- eccc %>%  
  mutate(ts = ymd_hm(paste(year, month, day, hour_start, minute_start),  
                     tz = "Canada/Yukon"),
         method = "PC", dur_min = 10, dur = 600) %>% rename(observer = observer_num)
...
###WCS point counts
wcs.det <- read.csv("E:/Archive/0_data/EW2021-JUL-21_Detections.csv")
...
saveRDS(species.abundance.l,  file.path(pipeline, "out", "spec_abun.RDS"))
...
st_write(sites, file.path(pipeline, "out", "sites.shp"), delete_layer=T)
saveRDS(sites, file.path(pipeline, "out", "sites.RDS"))
visits <- filter(visits, !(organization == "WCS" & method == "PC"))
write_csv(visits, file.path(pipeline, "out", "visits.csv"))
saveRDS(visits, file.path(pipeline, "out", "visits.RDS"))

write_csv(counts %>% mutate(largeHR = ifelse(species_code %in% species.HR$species_code, T, F)), file.path(pipeline, "out", "counts.csv"))
saveRDS(counts %>% mutate(largeHR = ifelse(species_code %in% species.HR$species_code, T, F)), file.path(pipeline, "out", "counts.RDS"))

write_csv(counts_sum, file.path(pipeline, "out", "counts_sum.csv"))
saveRDS(counts_sum, file.path(pipeline, "out", "counts_sum.RDS"))

#Mayo sites
sites %>% st_crop(xmin = 296800, ymin = 984000, xmax = 391600, ymax = 1106450) %>% st_write(dsn = "2_pipeline/1_CountDataProcessing/tmp/sitesMayo.shp", delete_layer=T)
...
write_csv(tmp, file.path(pipeline, "tmp", "counts2023.csv"))
